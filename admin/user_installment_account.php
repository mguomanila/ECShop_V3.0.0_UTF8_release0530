...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('user_ins_account_list.htm');
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($_REQUEST['act'] == 'edit')
    {
        ...
        /* 取得余额信息 */
        $user_account = $db->getRow("SELECT * FROM " .$ecs->table('installment_account') . " WHERE id = '$id'");
...
//  	$smarty->assign('stub_img',      $stub_img);
    ...
    assign_query_info();
    $smarty->display('user_installment_account_info.htm');
}
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    /* 页面显示 */
    assign_query_info();
    $smarty->display('user_ins_account_check.htm');
}
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    {
        ecs_header("Location: user_installment_account.php?act=list\n");
        exit;
    }
...
//	$integral  = $account['integral_amount'];
    ...
    if ($account['is_paid'] != 1)
    {
        ...
        {
			$sql = "UPDATE ".$ecs->table('installment_account')." SET is_paid = 1,paid_time = ".gmtime().",admin_note = '$admin_note',admin_user='$_SESSION[admin_name]' WHERE id = '$id'";
//			echo $sql;
			$db->query($sql);
			$sql = "UPDATE ".$ecs->table('installment')." SET num_arr = IF(num_arr,concat(num_arr,',','$account[num]'),'$account[num]')  WHERE id = '$account[installment_id]'";

			$db->query($sql);
        }elseif($is_paid == '2' && $account['process_type'] == '1')
        {
        	
			$sql = "UPDATE ".$ecs->table('installment_account')." SET is_paid = 2 WHERE id = '$id'";
//			echo $sql;
			$db->query($sql);
...
//      admin_log('(' . addslashes($_LANG['check']) . ')' . $admin_note, 'edit', 'user_surplus');
        ...
        /* 提示信息 */
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'user_installment_account.php?act=list&' . list_link_postfix();
...
//  }
    ...
    make_json_result($smarty->fetch('user_ins_account_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'remove')
{
	...
	$sql='SELECT * FROM ' .$ecs->table('installment_account').
    ...
    $sql = "DELETE FROM " . $ecs->table('installment_account') . " WHERE id = '$id'";
    if ($db->query($sql, 'SILENT'))
    {
       admin_log(addslashes($user_name), 'remove', 'user_surplus');
       $url = 'user_installment_account.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
//      }
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('installment_account'). " AS ua RIGHT JOIN ".
