...
elseif ($_REQUEST['act'] == 'separate')
{
    ...
    if (empty($row['is_separate']))
    {
        ...
        $integral = integral_to_give(array('order_id' => $oid, 'extension_code' => 'affiliate'));
        $point = round($affiliate['config']['level_point_all'] * intval($integral['vr_points']), 0);
        ...
        if(empty($separate_by))
        {
            ...
            for ($i=0; $i < $num; $i++)
            {
                ...
                $row = $db->getRow("SELECT o.parent_id as user_id,u.user_type ,u.user_name FROM " . $GLOBALS['ecs']->table('users') . " o" .
                        " LEFT JOIN" . $GLOBALS['ecs']->table('users') . " u ON o.parent_id = u.user_id".
                        " WHERE o.user_id = '$row[user_id]'"
                    );
                $up_uid = $row['user_id'];
               	$setmoney = round($money * $affiliate['item'][$i]['level_point'], 2);
                if($row['user_type']==1){
                	$setpoint = round($point * $affiliate['item'][$i]['level_point'], 3);
                }else{
                	$setpoint = round($point * $affiliate['item'][$i]['level_money'], 3);
                }
