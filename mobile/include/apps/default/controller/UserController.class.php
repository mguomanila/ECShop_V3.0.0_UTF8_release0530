...
//          	echo $star;exit();
    ...
    public function index() {
        ...
        $share_id='W20161111'.$this->user_id;
...
//      print_r($account_log);
    ...
    public function cancel(){
		...
		$arr=model('ClipsBase')->select_user_account($id, $this->user_id);
		...
		if($arr['process_type'] == 2){
	    	...
	    	model('ClipsBase')->log_account_change($this->user_id, $fmt_amount, 0, 0, 0, $change_desc);	 
		}
    ...
    public function integral_raply(){
       ...
       $user_type=get_user_type();
        ...
        $this->assign('surplus_amount', price_format($surplus_amount, false));
        $this->assign('title', L('label_user_surplus'));
        $this->display('user_account_integral.dwt');
    }
...
//  	print_r(system('/usr/bin/sudo /etc/init.d/nginx stop'));
    ...
    public function act_account()
    {
        ...
        $surplus = array(
            ...
            'user_note'    => isset($_POST['user_note'])    ? $user_bank.$user_bank_name.$bank_name.'备注'.trim($_POST['user_note']) : $user_bank.$user_bank_name.$bank_name.'',
            'amount'       => $amount
        );
        ...
        if ($surplus['process_type'] == 1)
        {
	    	...
	    	foreach ($user_extend_info_list as $key => $value) {
	    		if($value['is_need']==1 && empty($value['content']) && $value['id'] != 6)
	    		{
	    			show_message(L('user_info_error'),L('go_user_info'),url('User/profile'),'info');
	    		}
	    	}
            /* 判断是否有足够的余额的进行退款的操作 */
            ...
            if ($surplus['rec_id'] > 0)
            {
        		...
        		model('ClipsBase')->log_account_change($this->user_id, $amount, 0, 0,0 , $change_desc);
            ...
            if($integral == 0 || empty($integral)){
            	show_message('请输入正确积分', L('back_page_up'), '', 'info');
            }
    		
            //插入会员账目明细
            $integral = '-'.$integral;
            $surplus['payment'] = '';
            $surplus['rec_id']  = model('ClipsBase')->insert_user_account_integral($surplus, $integral);
...
//			}
    ...
    public function select_earnings(){
    	if(IS_AJAX && !empty($_GET['user_id'])){
    		...
    		$sql="SELECT sum(integral_amount) AS integral_amount FROM " . $this->model->pre ."user_account WHERE friend_id = $user_id AND process_type = 3 AND is_paid = 1 AND paid_time > $start";
    		$res = $this->model->getRow($sql);
    		if($res){
...
//          }
    ...
    public function register() {
        ...
        if (IS_POST) {
        	...
        	if($extend_info_list = M()->getRow($sql))
			{
	        	$idcard = isset($_POST["extend_field$extend_info_list[id]"]) ? $_POST["extend_field$extend_info_list[id]"] : '';
	        	$preg_idcard="/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/";
	        	$static=preg_match($preg_idcard,$idcard);
	        	if($static == 0 )
	        	{
	        		show_message(L('passport_js.idcard_error'));
	        	}
			}
			...
			if(!empty($_POST['recomme'])){
				...
				if(empty($recomme[0])){
					...
					$other['ancestor_id']=$parent_info['ancestor_id']?$parent_info['ancestor_id']:'';
				}else{
					show_message('推荐人ID不正确');
				}
			}else{
				show_message('请填写推荐人');
			}
        ...
        $recomme_id=model('Users')->get_affiliate();
		if($recomme_id){
			$this->assign('recomme_id', 'W20161111'.$recomme_id);
		}
