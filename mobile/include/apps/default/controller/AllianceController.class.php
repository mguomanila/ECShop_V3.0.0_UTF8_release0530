...
class AllianceController extends CommonController {
    ...
    public function index() {
        ...
        $count = model('Alliance')->get_brands_count();
        $this->pageLimit(url('index'), $this->size);

        $this->assign('pager', $this->pageShow($count));
// print_r(get_children(26));
            

$this->assign('category', model('CategoryBase')->get_categories_tree());

        $list = model('Alliance')->get_brands('brand', $this->size, $this->page);
        $this->assign('list', $list);

        $this->display('alliance_goods_list.dwt');
    }
...
$get_catid=isset($_GET['cat_id'])?$_GET['cat_id']:0;
        ...
        $list = model('Alliance')->get_brands('brand', $this->size, $this->page,$_GET['cat_id'],$_GET['cat_str']);
    ...
    public function goods_list() {
        ...
        $alliance_list = model('Alliance')->get_alliance_info($_REQUEST['id']);
        $facade_img=explode('|',$alliance_list['facade_img']);
        $this->assign('facade_img', $facade_img);
        $this->assign('alliance_list', $alliance_list);

        $alliance_user_list = model('Alliance')->get_alliance_user($alliance_list['suppliers_name']);
        $this->assign('alliance_user_list', $alliance_user_list);
        
        
        $goods_list = model('Alliance')->alliance_get_goods(0, '', $this->sort, $this->order, $this->size, $this->page);
        $this->assign('goods_list', $goods_list);
        $count = model('Alliance')->goods_count_by_brand(0);
        $this->pageLimit(url('goods_list', array('sort' => $this->sort, 'order' => $this->order)), $this->size);        
        $this->assign('pager', $this->pageShow($count));
		$this->assign('show_marketprice', C('show_marketprice'));
        $this->display('sjxq.dwt');
    }
