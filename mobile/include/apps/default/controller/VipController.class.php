...
class VipController extends CommonController {
    ...
    public function index() {
        ...
        $this->assign('vip_sta', true);
        
        // 获取分类
        $this->assign('category', model('CategoryBase')->get_top_category());
        $count = $this->category_get_count($this->children, $this->brand, $this->type, $this->price_min, $this->price_max, $this->ext,$this->keywords);
    ...
    function category_get_count($children,$brand, $type, $min, $max, $ext, $keyword) {
        ...
        $where .= " AND g.is_vip = 1 ";
        
        $sql = 'SELECT COUNT(*) as count FROM ' . $this->model->pre . 'goods AS g ' . ' LEFT JOIN ' . $this->model->pre . 'touch_goods AS xl ' . ' ON g.goods_id=xl.goods_id ' . ' LEFT JOIN ' . $this->model->pre . 'member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE $where $ext ";
