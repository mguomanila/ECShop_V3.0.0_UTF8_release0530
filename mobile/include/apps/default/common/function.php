...
$sql='SELECT u.user_type'.
            ' FROM ' . M()->pre .'users'. ' AS u ' .
            " WHERE u.user_id = '$id'";
			$user_type = M()->getRow($sql);
...
function get_goods_count($goods_id)
{
    ...
    $sql = "SELECT sum(goods_number) as num FROM " . M()->pre .'order_goods' . " AS g ,".M()->pre .'order_info' . " AS o WHERE o.order_id = g.order_id and g.goods_id = " . $goods_id . " and o.order_status = 5";
    $cum_sales = M()->getRow($sql);
    $sql_goods = "SELECT virtual_sales FROM " . M()->pre .'goods' . " WHERE goods_id = '$goods_id'";
    $cum= M()->getRow($sql_goods);
    $cum_sales['num']+=$cum['virtual_sales'];
    return $cum_sales['num'];
