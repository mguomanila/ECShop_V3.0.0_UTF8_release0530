...
if ($action == 'register')
{
	...
	$recomme_id=get_affiliate();

	if($recomme_id){
		$smarty->assign('recomme_id', '20161111'.$recomme_id);
	}
    /* 取出注册扩展字段 */
...
elseif ($action == 'act_register')
{
    ...
    else
    {
    	...
    	if($extend_info_list = $db->getRow($sql))
		{
        	...
        	if($static == 0 )
        	{
        		show_message($_LANG['passport_js']['idcard_error']);
        	}
		}
...
elseif ($action == 'profile')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $ecs->table('reg_fields') . ' WHERE type < 2 ORDER BY dis_order, id';
    $extend_info_list = $db->getAll($sql);
...
elseif ($action == 'act_account')
{
    ...
    if ($surplus['process_type'] == 1)
    {
    	...
    	foreach ($user_extend_info_list as $key => $value) {
    		...
    		{
    			show_message($_LANG['user_info_error'],$_LANG['go_user_info'],'user.php?act=profile','info');
    		}
    	}
        /* 判断是否有足够的余额的进行退款的操作 */
        ...
        if ($surplus['rec_id'] > 0)
        {
        	log_account_change($user_id, $amount, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
		...
		$amount = '-'.$integral;
        $surplus['payment'] = '';
        $surplus['rec_id']  = insert_user_account_integral($surplus, $amount);
    ...
    elseif($surplus['process_type'] == 3){
    	...
    	if($user_type==2){
	        ...
	        //记录支付log
	        $order['log_id'] = insert_pay_log($surplus['rec_id'], $order['order_amount'], $type=PAY_INTEGRAL, 0);
...
elseif ($action == 'cancel')
{
    ...
    if($arr['process_type'] == 2){
    	...
    	log_account_change($user_id, $fmt_amount, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
    }
...
elseif ($action == 'affiliate')
{
    ...
    if(empty($goodsid))
    {
        ...
        if(empty($affiliate['config']['separate_by']))
        {
			...
			$sql="SELECT * FROM " .$ecs->table('users') ." WHERE parent_id = $user_id";
			$aff_arr=$db->getAll($sql);
			$smarty->assign('aff_arr', $aff_arr);
