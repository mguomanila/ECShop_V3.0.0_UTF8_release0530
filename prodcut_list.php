...
if (!$smarty->is_cached('prodcut_list.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
...
$smarty->display('prodcut_list.dwt', $cache_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id,g.goods_sn, g.brand_id,g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$brand_sql='SELECT brand_name FROM'. $GLOBALS['ecs']->table('brand') .'WHERE brand_id = '.$row['brand_id'];
		$brand_name=$GLOBALS['db']->getOne($brand_sql);
		$arr[$row['goods_id']]['brand_id']             = $brand_name;
