 ...
 * 每月返现活动
 */
function fx_activity($fx_money){
	$change_time=gmtime();
	$year = date("Y");
	$month = date("m");
	$start = mktime(0,0,0,$month,1,$year);//当月开始时间戳


	$sql_user="SELECT * FROM". $GLOBALS['ecs']->table('users')." WHERE fx_activity <> 0";
	...
	foreach($user_row as $key => $value){
		...
		if(!$row){
			...
			if($user_row[0]['fx_activity'] > 0){
				if($user_row[0]['fx_activity']<$fx_money){
					$fx_activity_meiyue=$user_row[0]['fx_activity'];
				}else{
					$fx_activity_meiyue=$fx_money;
				}
			}else{
				$fx_activity_meiyue=0;
			}
		    ...
		    $sql_update = "UPDATE " . $GLOBALS['ecs']->table('users') .
	            " SET user_money = user_money + ('$fx_activity_meiyue')," .
	            " fx_activity = fx_activity - ('$fx_activity_meiyue')" .
	            " WHERE user_id = ".$value['user_id'] ." LIMIT 1";
...
function sm_fx_integral(){
	...
	foreach($user_row as $key => $value){
		...
		if(!$row){
			...
			"WHERE user_id=".$value['user_id'] ." LIMIT 1";
			$user_row=$GLOBALS['db']->getAll($sql_user_one);
			if($user_row[0]['vr_state']==1||$user_row[0]['vr_points'] >= 29850){
				$sql_vr_state="UPDATE". $GLOBALS['ecs']->table('users'). " set vr_state = 1 WHERE user_id=".$value['user_id'] ." LIMIT 1";
				
				$GLOBALS['db']->query($sql_vr_state);
				...
				if($pay_points != 0){
				    ...
				    /* 更新用户信息 */
				    $sql_update = "UPDATE " . $GLOBALS['ecs']->table('users') .
			            " SET vr_points = vr_points - ('$pay_points_100')," .
	...
	$sql_user="SELECT * FROM". $GLOBALS['ecs']->table('users')." WHERE fangan3<>0";
	$user_row=$GLOBALS['db']->getAll($sql_user);
	foreach ($user_row as $key => $value) {
		$sql="SELECT * FROM". $GLOBALS['ecs']->table('account_log').
		"WHERE user_id=".$value['user_id'] ."  AND change_desc = '方案3每日返积分'  AND change_time BETWEEN $start AND $change_time";
...
//				$fangan3=$user_row[0]['bili3'];
	...
	$sql_user="SELECT * FROM". $GLOBALS['ecs']->table('users')." WHERE fangan4<>0";
	$user_row=$GLOBALS['db']->getAll($sql_user);
	foreach ($user_row as $key => $value) {
		$sql="SELECT * FROM". $GLOBALS['ecs']->table('account_log').
		"WHERE user_id=".$value['user_id'] ."  AND change_desc = '方案4每日返积分'  AND change_time BETWEEN $start AND $change_time";
...
//				$fangan3=$user_row[0]['bili3'];
			    ...
			    $account_log = array(
			        ...
			        'frozen_money'  => 0,
			        'rank_points'   => 0,
			        'pay_points_4'    => $pay_points,
			        'fangan4'    		=> (-1)*$pay_points_100,
			        'change_time'   => gmtime(),
			        'change_desc'   => '方案4每日返积分',
			        'change_type'   => $type
			    );
			    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $account_log, 'INSERT');
			    ...
			    /* 更新用户信息 */
			    $sql_update = "UPDATE " . $GLOBALS['ecs']->table('users') .
		            " SET fangan4 = fangan4 - ('$pay_points_100')," .
		            " pay_points_4 = pay_points_4 + ('$pay_points')" .
		            " WHERE user_id = ".$value['user_id'] ." LIMIT 1";
			    $GLOBALS['db']->query($sql_update);
		   	}
