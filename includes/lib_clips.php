...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
			...
			if($rows['is_paid'] == 0){
				$rows['pay_status'] = $GLOBALS['_LANG']['un_confirm'];
			}elseif($rows['is_paid'] == 1){
				$rows['pay_status'] = $GLOBALS['_LANG']['is_confirm'];
			}else{
				$rows['pay_status'] = $GLOBALS['_LANG']['is_cancel'];
			}
            ...
            elseif($rows['process_type'] == 3)
            {
            	if(empty($rows['friend_id'])){
            		$rows['type'] = $GLOBALS['_LANG']['surplus_type_3'].':自己';
            	}else{
...
function get_user_field_info($user_id)
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
	
	$user_info = get_profile($user_id);
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('reg_fields') . ' WHERE type < 2 ORDER BY dis_order, id';
